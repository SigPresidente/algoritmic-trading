...
def main():
    for sym in SYMBOLS:
        ...
        for profile in PROFILES:
            ...
            # Total return over N days: (1 + daily_rate)^N = (1 + total_return)
            # So: daily_rate = (1 + total_return)^(1/N) - 1
            ...
            df['robo_equity'] = INITIAL_DEPOSIT * (1 + daily_return_rate) ** np.arange(days_in_backtest)
            ...
            all_profiles_data[profile] = {
                ...
                'algo_vol': algo_vol,
                'bench_vol': bench_vol,
                'robo_vol': robo_volatilities[profile],
        ...
        # 1)Equity Curves (for more than one symbol)
        ...
        # Plot robo-advisor for each profile
        for profile in PROFILES:
            data = all_profiles_data[profile]
            df = data['df']
            ax.plot(df.index, df['robo_equity'], label=f'Moneyfarm - {profile_labels[profile]}', 
                   linewidth=2, color=colors[profile], linestyle='--', alpha=0.7)
        
        # Add benchmark (performance of specific symbol)
        bench_df = all_profiles_data[PROFILES[0]]['df']
        ax.plot(bench_df.index, bench_df['close'] / bench_df['close'].iloc[0] * INITIAL_DEPOSIT, 
               label=f'Benchmark ({sym})', linewidth=2.5, color='black', linestyle=':', alpha=0.8)
        
        ax.set_title(f'Equity Curve Comparison - Algorithm vs Moneyfarm vs Benchmark - {sym}', 
        ...
        # 2)Cumulative Returns (for more than one symbol)
        fig, ax = plt.subplots(figsize=(16, 8))
        ...
        ax.set_title(f'Cumulative Returns Comparison - Algorithm vs Moneyfarm vs Benchmark - {sym}', 
                    fontsize=16, fontweight='bold', pad=20)
        ax.set_xlabel('Date', fontsize=13, fontweight='bold')
        ...
        # 3)Drawdown Curves - All Profiles
        fig, ax = plt.subplots(figsize=(16, 8))
        ...
        for profile in PROFILES:
            ...
            ax.plot(df.index, df['drawdown'], label=f'{profile_labels[profile]}', 
                   linewidth=2.5, color=colors[profile], alpha=0.9)
            ax.fill_between(df.index, df['drawdown'], 0, color=colors[profile], alpha=0.2)
        
        ax.set_title(f'Drawdown Comparison - All Risk Profiles - {sym}', 
                    fontsize=16, fontweight='bold', pad=20)
        ax.set_xlabel('Date', fontsize=13, fontweight='bold')
        ...
        fig.suptitle(f'Trade P/L Distribution - All Risk Profiles - {sym}', 
        ...
        fig.suptitle(f'Performance Metrics Comparison - Algorithm vs Moneyfarm - {sym}', 
                    fontsize=16, fontweight='bold', y=0.995)
        
        profiles_list = list(all_profiles_data.keys())
        x = np.arange(len(profiles_list))
        width = 0.35
        
        # Metric 1: Total Return (Algo vs Robo)
        algo_returns = [all_profiles_data[p]['metrics']['Total Return (%)'] for p in profiles_list]
        robo_returns = [all_profiles_data[p]['robo_total_return'] for p in profiles_list]
        
        bars1 = axes[0, 0].bar(x - width/2, algo_returns, width, label='Algorithm', 
                              color=[colors[p] for p in profiles_list], alpha=0.8)
        bars2 = axes[0, 0].bar(x + width/2, robo_returns, width, label='Moneyfarm', 
                              color=[colors[p] for p in profiles_list], alpha=0.5, hatch='//')
        ...
        axes[0, 0].set_xticklabels([profile_labels[p] for p in profiles_list], fontsize=10)
        axes[0, 0].legend(fontsize=10)
        axes[0, 0].grid(True, alpha=0.3, axis='y')
        axes[0, 0].axhline(y=0, color='gray', linestyle='-', linewidth=0.8)
        
        # Metric 2: Sharpe Ratio
        sharpes = [all_profiles_data[p]['metrics']['Sharpe Ratio'] for p in profiles_list]
        bars = axes[0, 1].bar(x, sharpes, width*2, color=[colors[p] for p in profiles_list], alpha=0.8)
        for bar in bars:
            height = bar.get_height()
            axes[0, 1].text(bar.get_x() + bar.get_width()/2., height,
                           f'{height:.2f}', ha='center', va='bottom' if height >= 0 else 'top', 
                           fontsize=10, fontweight='bold')
        axes[0, 1].set_title('Sharpe Ratio', fontsize=13, fontweight='bold')
        axes[0, 1].set_xticks(x)
        axes[0, 1].set_xticklabels([profile_labels[p] for p in profiles_list], fontsize=10)
        axes[0, 1].grid(True, alpha=0.3, axis='y')
        axes[0, 1].axhline(y=0, color='gray', linestyle='-', linewidth=0.8)
        
        # Metric 3: Max Drawdown
        drawdowns = [all_profiles_data[p]['metrics']['Max Drawdown (%)'] for p in profiles_list]
        bars = axes[1, 0].bar(x, drawdowns, width*2, color=[colors[p] for p in profiles_list], alpha=0.8)
        for bar in bars:
            height = bar.get_height()
            axes[1, 0].text(bar.get_x() + bar.get_width()/2., height,
                           f'{height:.1f}%', ha='center', va='top', 
                           fontsize=10, fontweight='bold')
        axes[1, 0].set_title('Max Drawdown (%)', fontsize=13, fontweight='bold')
        axes[1, 0].set_xticks(x)
        axes[1, 0].set_xticklabels([profile_labels[p] for p in profiles_list], fontsize=10)
        axes[1, 0].grid(True, alpha=0.3, axis='y')
        
        # Metric 4: Win Rate
        win_rates = [all_profiles_data[p]['metrics']['Win Rate (%)'] for p in profiles_list]
        bars = axes[1, 1].bar(x, win_rates, width*2, color=[colors[p] for p in profiles_list], alpha=0.8)
        for bar in bars:
            height = bar.get_height()
            axes[1, 1].text(bar.get_x() + bar.get_width()/2., height,
                           f'{height:.1f}%', ha='center', va='bottom', 
                           fontsize=10, fontweight='bold')
        axes[1, 1].set_title('Win Rate (%)', fontsize=13, fontweight='bold')
        axes[1, 1].set_xticks(x)
        axes[1, 1].set_xticklabels([profile_labels[p] for p in profiles_list], fontsize=10)
        ...
        # 6)Volatility Comparisons
        fig, ax = plt.subplots(figsize=(14, 7))
        
        profiles_list = list(all_profiles_data.keys())
        algo_vols = [all_profiles_data[p]['algo_vol'] for p in profiles_list]
        bench_vols = [all_profiles_data[p]['bench_vol'] for p in profiles_list]
        robo_vols = [all_profiles_data[p]['robo_vol'] for p in profiles_list]
        
        x = np.arange(len(profiles_list))
        width = 0.25
        
        bars1 = ax.bar(x - width, algo_vols, width, label='Algorithm', color='#2E86AB', alpha=0.8)
        bars2 = ax.bar(x, bench_vols, width, label=f'Benchmark ({sym})', color='#A23B72', alpha=0.8)
        bars3 = ax.bar(x + width, robo_vols, width, label='Moneyfarm (P1/P4/P7)', color='#F18F01', alpha=0.8)
        ...
        ax.set_title(f'Volatility Comparison - Algorithm vs Benchmark vs Moneyfarm - {sym}', 
                    fontsize=15, fontweight='bold', pad=20)
        ax.set_xticks(x)
        ax.set_xticklabels([profile_labels[p] for p in profiles_list], fontsize=11)
        ax.legend(fontsize=11, loc='upper left')
        ax.grid(True, alpha=0.3, axis='y')
        
        ax.text(0.98, 0.02, 'Note: Moneyfarm volatilities are estimates based on portfolio characteristics',
    ...
    print("\n" + "="*60)
    print("All combined graphs and metrics updated and saved.")
    print("="*60)
